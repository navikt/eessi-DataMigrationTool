...
/**
 * Tests the creation of DOC_RECEIVE actions for X008
 */
@Transactional
public class ReceiveX008ActionProducerTest extends AbstractBucProcessorTest {

    private static final Logger logger = LoggerFactory.getLogger(ReceiveX008ActionProducerTest.class);

    @Autowired
    private BucDefinitionImporterFactory bucDefinitionImporterFactory;

    @Autowired
    private X008ReceiveActionProducer x008ReceiveActionProducer;
    ...
    protected List<ActionDO> getReceiveUpdateActions(final RinaCase rinaCase, final Case bucDefinition) {
        ...
        // get the docs
        List<Document> receivedDocs = x008ReceiveActionProducer.getDocumentsToProcess(rinaCase, bucDefinition);

        // get the actions
        return x008ReceiveActionProducer.getReceiveActions(rinaCase.getId(), receivedDocs, bucDefinition);
