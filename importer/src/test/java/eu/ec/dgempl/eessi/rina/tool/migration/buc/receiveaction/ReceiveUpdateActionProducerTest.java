 ...
 */
@Transactional
public class ReceiveUpdateActionProducerTest extends AbstractBucProcessorTest {

    private static final Logger logger = LoggerFactory.getLogger(ReceiveUpdateActionProducerTest.class);

    @Autowired
    private BucDefinitionImporterFactory bucDefinitionImporterFactory;

    @Autowired
    private ReceiveUpdateActionProducer receiveUpdateActionProducer;
    ...
    protected List<ActionDO> getReceiveUpdateActions(final RinaCase rinaCase, final Case bucDefinition) {
        ...
        // get the docs
        List<Document> receivedDocs = receiveUpdateActionProducer.getReceivedDocuments(rinaCase);

        // get the actions
        return receiveUpdateActionProducer.getReceiveActions(rinaCase.getId(), receivedDocs, bucDefinition);
