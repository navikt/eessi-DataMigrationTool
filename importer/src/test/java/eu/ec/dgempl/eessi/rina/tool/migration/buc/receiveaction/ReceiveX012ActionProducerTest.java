 ...
 */
@Transactional
public class ReceiveX012ActionProducerTest extends AbstractBucProcessorTest {

    private static final Logger logger = LoggerFactory.getLogger(ReceiveX012ActionProducerTest.class);

    @Autowired
    private BucDefinitionImporterFactory bucDefinitionImporterFactory;

    @Autowired
    private X012ReceiveActionProducer x012ReceiveActionProducer;
    ...
    protected List<ActionDO> getReceiveUpdateActions(final RinaCase rinaCase, final Case bucDefinition) {
        ...
        // get the docs
        List<Document> receivedDocs = x012ReceiveActionProducer.getDocumentsToProcess(rinaCase, bucDefinition);

        // get the actions
        return x012ReceiveActionProducer.getReceiveActions(rinaCase.getId(), receivedDocs, bucDefinition);
